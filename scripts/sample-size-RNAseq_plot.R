...
for (filename in filenames) {
  ...
  power <- dat$power %>%
    group_by(alpha, method, selection) %>%
    summarise(
      estimate = mean(power, na.rm = TRUE),
...
p <- ggplot(lev, aes(
  ...
  facet_grid(SNR ~ N, labeller = label_bquote(
    cols = N:.(N),
    rows = SNR:.(SNR),
  )) +
  geom_point() +
...
plotname <- sprintf(
  "fig-S7_JER-control_sample-size_%s_%s.pdf",
  technology, data_set
)
print(plotname)
...
plotname <- sprintf(
  "fig-S8_power_sample-size_%s_%s.pdf",
  technology, data_set
)
print(plotname)
pathname <- file.path(fig_path, plotname)
