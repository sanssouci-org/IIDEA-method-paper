...
res <- fit(obj,
  B = 1000, alpha = alpha, family = "Simes",
...
# - - - - - - - - - - - - - - -
res_ARI <- fit(obj,
  B = 0, family = "Simes", alpha = alpha / pi0_hat,
  rowTestFUN = rowWilcoxonTests
)
...
p <- plotConfCurve(conf_bounds, xmax = 2.5 * max(n_DEG), cols = cols) +
  ...
  geom_vline(
    xintercept = xBH, linetype = "dotted",
    color = "red", size = 0.5
  )
